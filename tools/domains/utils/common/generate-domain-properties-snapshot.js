...
const create = async (schema, config = {}) => {
  ...
  try {
    ...
    await generateDomainSnapshot(schema, finalConfig);
 ...
 * @returns {Promise<void>}
 */
const generateDomainSnapshot = async (schema, config) => {
...
const generateInterfaceFile = async (
  ...
  const lines = [`export interface ${interfaceType}${className}Props {`];
  ...
  for (const col of domainColumns) {
    ...
    const isRequired = interfaceType === 'Snapshot' ? col.nn : false;
