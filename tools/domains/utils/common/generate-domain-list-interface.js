  ...
  for (const [tableId, table] of Object.entries(tables)) {
    ...
    try {
      ...
      const lines = [
        ...
        `export interface List${className}Props {`,
      ];
      ...
      const outputFile = path.join(
        ...
        'domain',
        'properties',
        `list-${kebabCase(name)}.model.ts`,
      );
      if (schema.excluded?.includes(`list-${kebabCase(name)}.model.ts`)) {
        logger.info(
          `Skipping generation of list-${kebabCase(name)}.model.ts as it is excluded.`,
        );
        continue;
      }
