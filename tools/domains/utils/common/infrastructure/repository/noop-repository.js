 ...
 */
const generateNoopMethodSignature = (methodName, parameters, returnType) => {
  const lines = [];
  const eslintDisable =
    parameters.length > 1
...
function noopRepositoryGet(schema, table) {
  const { className, primaryCol } = getTableProperties(schema, table);
  const lines = [];

  lines.push('');
  lines.push(
    ...generateNoopMethodSignature(
      'get',
...
function noopRepositoryGetByCodes(schema, table) {
  const { className, primaryCol } = getTableProperties(schema, table);
  const lines = [];
...
function noopRepositoryList(schema, table) {
...
function noopRepositorySave(schema, table) {
  const { className, primaryCol } = getTableProperties(schema, table);
  const lines = [];
...
function noopRepositoryDelete(schema, table) {
  const { className, primaryCol } = getTableProperties(schema, table);
  const lines = [];
