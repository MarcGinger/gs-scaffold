...
/**
 * Redis service optimized for product projections and read models
 *
 * Features:
 ...
 */
@Injectable()
export class ProductRedisProjectionService implements OnModuleDestroy {
  ...
  constructor(
    ...
    this.client = new Redis({
      ...
      port: parseInt(
        this.configManager.get('REDIS_PORT', '6379') || '6379',
        10,
      ),
      password: this.configManager.get('REDIS_PASSWORD'),
      db: parseInt(
        this.configManager.get('REDIS_PROJECTIONS_DB', '1') || '1',
        10,
      ), // Separate DB for projections
      keyPrefix: this.configManager.get('REDIS_KEY_PREFIX', 'gs:'),
      maxRetriesPerRequest: 3,
      lazyConnect: true,
    });

    this.keyPrefix = 'product-projections:';

    Log.info(this.baseLogger, 'ProductRedisProjectionService initialized', {
      component: 'ProductRedisProjectionService',
      method: 'constructor',
  ...
  async storeProjection<T = any>(
    ...
    Log.debug(this.baseLogger, 'product.projection.stored', {
      component: 'ProductRedisProjectionService',
  ...
  async getProjection<T = any>(
    ...
    try {
      ...
      Log.error(
        this.baseLogger,
        error as Error,
        'product.projection.parse.failed',
        {
          component: 'ProductRedisProjectionService',
  ...
  async queryProjections<T = any>(
    ...
    Log.debug(this.baseLogger, 'product.projections.queried', {
      component: 'ProductRedisProjectionService',
  ...
  async deleteProjection(
    ...
    Log.debug(this.baseLogger, 'product.projection.deleted', {
      component: 'ProductRedisProjectionService',
   ...
   */
  async bulkStoreProjections<T = any>(
    projectionType: string,
    records: Array<{
      id: string;
      data: T;
    ...
    for (const record of records) {
      ...
      const projectionRecord: ProjectionRecord<T> = {
        id: record.id,
        type: projectionType,
        data: record.data,
        metadata: {
    ...
    Log.debug(this.baseLogger, 'product.projections.bulk.stored', {
      component: 'ProductRedisProjectionService',
